...
export default class extends Vue {
  ...
  public async getData() {
    ...
    if (this.syncCourseBaseId != 0) {
      ...
      this.baseInfoData.pic =
        `${process.env.VUE_APP_SERVER_PICSERVER_URL}` + this.baseInfoData.pic
    }
  }
  ...
  public async saveData(): Promise<boolean> {
    return new Promise(async (resolve, reject) => {
      ...
      if (valid) {
        //图片路径去掉网址
        this.baseInfoData.pic = this.baseInfoData.pic.replace(
          `${process.env.VUE_APP_SERVER_PICSERVER_URL}`,
          ''
        )
