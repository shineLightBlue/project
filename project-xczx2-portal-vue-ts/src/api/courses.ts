...
// 媒资绑定
export async function mediaAssociation(
  mediaId: string,
  fileName: string,
  teachplanId: number
) {
...
export async function mediaUnAssociation(
  ...
  await createAPI(
    `/content/teachplan/association/media/${teachplanId}/${mediaId}`,
    'delete',
    {}
  )
}
...
export async function getTeachers(
  ...
  for (let key in data) {
    if (data[key].photograph) {
      data[key].photograph =
        `${process.env.VUE_APP_SERVER_PICSERVER_URL}` + data[key].photograph
    }
  }
