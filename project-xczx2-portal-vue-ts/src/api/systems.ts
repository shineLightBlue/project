...
// 列表
export async function dictionaryAll(
  params: any = undefined,
  body: any = undefined
): Promise<ISystemDictionary[]> {
  //const { data } = await createAPI('/system/dictionary/all', 'get', params, body)
  const { data } = await createAPI(
    'http://localhost:63110/system/dictionary/all',
    'get',
    params,
    body
  )
...
export function getDictByCode(code: string): IKVData[] {
  let item: ISystemDictionary[] = SystemModule.dicts.filter(
    (value: ISystemDictionary) => {
      if (value.code == code) {
        return value
      }
    }
